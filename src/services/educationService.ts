...
export async function getAllEducationItems(): Promise<EducationItem[]> {
  try {
    if (!STRAPI_API_TOKEN) {
      console.warn('STRAPI_API_TOKEN not found. Using public API...');
    }
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch education items: ${response.statusText}`);
    }
...
export async function getEducationItemBySlug(slug: string): Promise<EducationItem | null> {
  try {
    ...
    if (!response.ok) {
      throw new Error(`Failed to fetch education item: ${response.statusText}`);
    }
